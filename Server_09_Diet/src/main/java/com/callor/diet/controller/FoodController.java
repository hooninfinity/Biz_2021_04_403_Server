...
public class FoodController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (subPath == null || subPath.equals("")) {
			...
			ReqController.forward(req, resp, "insert"); // insert 화면을 열어라
		}
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (subPath == null || subPath.equals("")) {
			...
			List<FoodDTO> foodList = fdService.findByFName(f_name);//search.jsp의 변수와 같게
			
			req.setAttribute("FOODS", foodList); // 이름이 "FOODS"인 속성의 값을 foodList로 지정
			ReqController.forward(req, resp, "search");
