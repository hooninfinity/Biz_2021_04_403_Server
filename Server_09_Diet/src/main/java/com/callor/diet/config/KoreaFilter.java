 ...
 * 														-- 필터가 없으면 Controller.doGet() 또는 Controller.doPost()로 진행
 * 														-- 필터가 있으면 필터를 거쳐 Controller로 감
 * 
 * Controller에서 실제 업무와 연계되면서
 * 모든(많은) Controller에서 공통으로 설정해야 하는 것들 또는
 * 공통으로 처리해야 하는 것들을
 * 
 ...
 * login 여부에 따라 Controller로 전달할지 안할지를 판단하여 미리 한번에 처리할 수 있다.
 */
 ...
 * 	text/html 로 지정을 하면
 * 	jsp, js, 기타 여러가지 static 파일들이 모두 html ContentType으로 지정되어
 * 	WebBrowser로 전송되어 제기능을 수행하지 못하는 현상이 발생한다.
 * 	
 * 	이럴때는 WebFilter urlPatterns를 문자열 배열로 생성하고
 * 	각 Controller에 지정된 WebServlet 필터를 나열해 주면
 * 	Controller 로 요청되는 사항들만 Filtering을 하게 되어
 * 
...
public class KoreaFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		 ...
		 * 
		 * Controller에서 Web 으로 응답하는 데이터들의 ContentType을 설정하기 
		 */
