 ...
 * 똑같은 객체를 상속받고, 같은 method(doGet, doPost)를 사용하여 코드를 작성하는데
 * 관리가 어려워지기 시작한다.
 * 
 * URI mapping을 한곳으로 집중하고
 * POJO
 * 	(Plan Old Java Object, 어떤 클래스를 상속받지 않은 일반적인 자바 클래스 형식의 코드)를
 * 	사용하여 프로젝트를 진행하도록 하는 것
...
public class FrontController extends HttpServlet {
	...
	// TodoCommand 인터페이스는 command 객체들의 prototype으로 선언되고 있고
	// 		여기의 command mapper 객체에는 여러가지 command 객체들을 저장해 둘 수 있다.
	// 인터페이스를 사용하지 않고 Object를 사용할 수 있지만
	// 		Object 클래스는 상대적으로 연산비용이 많이 소요되는 클래스 객체이므로
	// 		조금더 효율적으로 사용하기 위하여 인터페이스를 선언해 두었다.
	protected Map<String, TodoCommand> commands;

	// FrontController가 최초 호출될때 한번 실행되어서 여러가지 변수 등을 초기화 하는 코드
	@Override
	public void init(ServletConfig config) throws ServletException {
		commands = new HashMap<String, TodoCommand>();
		
		/*
		 * 만약 http://localhost:8080/todo 로 요청이 오면(/roothPath)
