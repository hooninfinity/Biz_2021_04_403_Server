...
public class TodoServiceImplV1 implements TodoService {
	 ...
	 * 
	 * 메타데이터(meta data)
	 * 	생성한 Database, table, sequence, view... 
	 * 	어떤 구조로 만들어져 있는가 등의 정보를 메타데이터 라고 한다.
	 * 	예) myDB라는 database에 몇개의 table이 있는가 등의 정보를 알고 싶으면 myDB 메타데이터를 참조하면 된다
	 */
	protected List<Map<String, Object>> select(ResultSet rSet) throws SQLException {
		
		// ResultSet으로 메타데이터를 추출하여
		// 조회를 수행한 결과를 전달받은 Table(tbl_todolist)에
		// 몇개의 칼럼이 있는지 확인한다.
		ResultSetMetaData md = rSet.getMetaData();
		int columns = md.getColumnCount();

		// 먼저 Map을 이용한 List를 만들고
	...
	public List<Map<String, Object>> selectAll() {
		...
		try {
			...
			/* 잘 실행되나 콘솔에 출력하기 위한 테스트 구문
			// 메타데이터를 추출하고, 칼럼의 갯수를 가져오고,
			ResultSetMetaData md = rSet.getMetaData();
