...
public class BookRentServiceImplV1 implements BookRentService{
	...
	public BookRentServiceImplV1() {
		dbConn = DBContract.getDBConnection(); // 데이터 연결
	}
	
	/*
	 * exception 처리 방법 2가지
	 * 1. try-catch 를 사용하기
	 * 		exception이 발생할 코드를 감싸서 "직접" exception 을 처리하기
	 * 
	 * 2. 떠넘기기(던지기)
	 * 		method throws 를 추가하여
	 * 		호출한 method에게 exception을 되돌려 보내기
	 * 		현재 method 에서는 직접 exception 을 처리하는 코드가 없어서 코드가 다소 간소화 된다
	 * 		
	...
	public BookRentDTO findById(Long seq) {
		 ...
		 * 실제 쿼리는 WHERE 도서명 = 1 OR 1 = 1 과 같이 만들어져 버린다
		 * 
		 * PreparedStatement 를 사용할때는 조건문의 변수가 포함될 위치에
		 * ? 기호를 사용한다
		 * WHERE 도서명 = ? 와 같이 사용한다
		 */
		String sql = " SELECT * FROM view_도서대여정보 ";
		...
		try {
			// 아래의 2개 코드에 의해서
			//		WHERE 주문번호 = seq 값 형식의 SQL이 만들어진다
			pStr = dbConn.prepareStatement(sql);
			pStr.setLong(1, seq);
...
//			rSet.close();
			...
			// PK로 조회를 했기 때문에
			// List에는 1개밖에 데이터가 없다
			// list의 0 번 데이터만 getter하여 DTO에 담기
			List<BookRentDTO> brList = this.select(pStr);
