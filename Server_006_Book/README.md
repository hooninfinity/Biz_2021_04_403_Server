...
* MVC 패턴의 프로젝트에서는 모든 요청을 만드시 Controller 클래스에서 처리하고, 사용자에게 보여주는 부분은 jsp 파일에서 처리하도록 작성을 한다.
...
* MVS 패턴에서는 jsp 파일을 직접 호출하지 못하도록 한다.
* WEB-INF 폴더아래에 views 폴더를 만들고 jsp를 저장하면 web browser 에서 jsp를 직접 호출할 수 없게 된다. WEB-INF는 프로젝트의 Controller 클래스에서만 접근할 수 있도록 되어 있다.
