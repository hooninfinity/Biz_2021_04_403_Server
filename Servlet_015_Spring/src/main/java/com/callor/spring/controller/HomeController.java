...
public class HomeController {
	 ...
	 * 
	 * 전통적인 Java 클래스간의 연결은
	 * 인터페이스로 객체를 선언하고 클래스로 객체를 생성(초기화)하여 가지고 있다가
	 * 		필요한 method를 호출할 수 있도록 만들어진다.
	 * 
	 * 하지만 SpringFramework 환경에서는 객체를 미리 Spring이 만들어 보관하고 있다가
	 * 		필요한 곳이 있으면 그때그때 배분하여 사용할 수 있도록 만들어준다.
	 * 
	 * 프로젝트가 커져서 많은 객체가 필요할때
	 * 		전통적 프로그래밍은 모든 객체를 선언 생성하여 가지고(메모리에 보관)있어야 하기 때문에
	 * 		어느정도 프로젝트가 커지면 여러가지 관리해야 할 일들이 많아진다.
	 * 
	 * SpringFramework 환경에서는 그러한 것들을 극복하기 위하여
	 * 		미리 객체를 만들어서 Spring 컨테이너 라는 곳에 보관해 두고
	 * 		필요한 곳에 적절히 주입을 해준다.
	 * 		
